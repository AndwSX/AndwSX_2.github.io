// UI Manager for the cutting simulator

// Define SimulatorParams class
...
class UIManager {
  ...
  getSimulatorParams() {
    return new SimulatorParams(
      Number.parseFloat(this.cutThicknessInput.value) || 0.3,
      Number.parseFloat(this.marginInput.value) || 0,
  ...
  renderPieces(pieces, onUpdate, onDelete) {
    ...
    if (pieces.length === 0) {
      this.piecesContainer.innerHTML =
        '<div class="text-center py-4 text-muted">No hay piezas. Añade una pieza para comenzar.</div>'
      return
    }
    ...
    pieces.forEach((piece) => {
      ...
      pieceCard.querySelector(`.save-piece[data-id="${piece.id}"]`).addEventListener("click", () => {
        const updatedPiece = new Piece(
          ...
          Number.parseFloat(pieceCard.querySelector(".piece-width").value),
          Number.parseFloat(pieceCard.querySelector(".piece-height").value),
          Number.parseInt(pieceCard.querySelector(".piece-quantity").value),
          piece.color,
        )
  ...
  renderBoards(boards, onUpdate, onDelete) {
    ...
    if (boards.length === 0) {
      this.boardsContainer.innerHTML =
        '<div class="text-center py-4 text-muted">No hay tablas. Añade una tabla para comenzar.</div>'
      return
    }
    ...
    boards.forEach((board) => {
      ...
      boardCard.querySelector(`.save-board[data-id="${board.id}"]`).addEventListener("click", () => {
        const updatedBoard = new Board(
          ...
          Number.parseFloat(boardCard.querySelector(".board-width").value),
          Number.parseFloat(boardCard.querySelector(".board-height").value),
